...
# Task 2 - Generation
This OCR pipeline implements a CNN-BiLSTM-CTC architecture for recognizing text from CAPTCHA-like images. The system begins by defining a vocabulary of 62 alphanumeric characters plus an '@' symbol, with an additional blank token for CTC loss, totaling 63 classes. Images are loaded through a custom Dataset class that extracts labels from filenames, converts images to grayscale, and resizes them to 100×200 pixels. The core model uses a modified ResNet18 backbone where the first convolutional layer is adjusted to accept single-channel grayscale input, and the final classification layers are removed to preserve spatial features. The CNN outputs feature maps that are then reduced from 512 to 256 channels via a 1×1 convolution. These features are reshaped from (batch, channels, height, width) to (width, batch, channels×height), effectively converting the image into a sequence where each column represents a time step with 1024-dimensional features. This sequence is processed by a 2-layer bidirectional LSTM with 256 hidden units per direction, capturing contextual information from both left-to-right and right-to-left. The LSTM outputs are passed through a linear layer to produce per-time-step character probabilities. The model is trained using CTC loss, which allows alignment-free learning between variable-length input sequences and output labels. Training runs for 30 epochs with Adam optimizer, showing decreasing loss from 2.9 to 0.003. During inference, predictions are decoded by taking the argmax at each time step, removing repeated characters and blank tokens. Evaluation metrics—character accuracy (29.7%), word accuracy (14.8%), and character error rate (70.4%)—indicate poor performance, likely due to bugs in character mapping, tensor shape handling, and label processing that need correction for the model to function properly. Despite these implementation issues, the architecture itself represents a standard end-to-end OCR approach suitable for sequence recognition tasks without explicit character segmentation.
